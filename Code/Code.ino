...
// ============= НАСТРОЙКИ WiFi =============
const char* ssid = "iPhone (Бейбарыс) (2)";           // Замените на имя вашей WiFi сети
const char* password = "Ernur228";     // Замените на пароль WiFi

// ============= НАСТРОЙКИ СЕРВЕРА =============
const char* serverURL = "http://172.20.10.2:5000/receive_data";  // Замените IP на адрес вашего компьютера
...
// ============= НАСТРОЙКИ ДАТЧИКА =============
const int FLOW_SENSOR_PIN = 27;  // Пин для датчика потока воды
volatile int pulseCount = 0;     // Счетчик импульсов от датчика
float calibrationFactor = 15;   // Калибровочный коэффициент (импульсов на литр)
...
unsigned long timePassed = millis() - oldTime;

// Расход (литров в минуту)
flowRate = (pulseCount / 450.0) * (60000.0 / timePassed);  // 450 импульсов ≈ 1 литр

// Литры за этот интервал
